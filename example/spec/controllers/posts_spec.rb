...
describe Posts, "#index" do
  ...
  describe 'going to page 2 with 21 objects' do
    before(:each) do
    ...
    it "should support regex checks" do
      @content.should match(/<ul.*?>(.*)<\/ul>/m)
        @content =~ /<ul.*?>(.*)<\/ul>/m 
        submatch = $1
    ...
    it "should have pagination control" do
       @content.should have_selector('div[@class=pagination]')
        #links enabled/disabled based on first/last page
    ...
    it "should have pagination control still with merby checks" do
    ...
    it "should have pagination control still with merby/hpricot checks" do
       @content.should have_tag('div',:class=>'pagination') do |inner|
           does_include_prev = nil
           does_include_next = nil
           (inner / 'a').each do |tag|
